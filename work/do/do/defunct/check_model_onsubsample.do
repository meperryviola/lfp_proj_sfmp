** Try estimating the imputation regression on the subset of the population who has ftotval<=25000
...
keep if ftotval<=25000  // comment this line in and out if you want to estimate for a subgroup or not.
...
/** test regression
regress lnftotval age i.elderly i.hefaminc i.max_educ i.min_educ i.familytype##c.numadult i.sex##i.marital i.race c.numkid##i.under6 i.spousemp i.peafever
...
regress lnftotval hh_lfp age age2 i.elderly i.hefaminc i.race##i.max_educ i.min_educ i.spousemp i.peafever i.hrhtype c.numadult i.sex##i.marital c.numkid##i.under6 i.h_tenure
...
foreach varname in `varlist'{
	...
	*replace `varname' = phat_`varname' if `varname'==. // this fills in the original with the imputed values for the ones previously missing, letting the ASEC ppl keep their actual values.
	replace ln`varname' = phat_ln`varname' if ln`varname'==. & is_basic==1 // don't attribute any for asec people
...
save loglinear_test_`x', replace
}


use loglinear_test_2019, clear
twoway (histogram unlog_phat_ftotval if ftotval<25000, width(1000) color(red%30)) (histogram unlog_ftotval if ftotval<25000, width(1000) color(green%30))

/*
append using loglinear_test_2020, force
append using loglinear_test_2021, force
...
* total distribution 
twoway (histogram unlog_phat_ftotval if unlog_phat_ftotval<200000, width(10000) color(red%30)) (histogram unlog_ftotval if unlog_ftotval<200000, width(10000) color(green%30))
