...
g numkids = (a_age<18)
bys year hrhhid hrhhid2: egen h_numkids = sum(numkids)

* household-level variable for EIP
bys year hrhhid hrhhid2: egen heip = sum(eip_crd)

* make household-level variable for Advance ctc amount
bys year hrhhid hrhhid2: egen h_adv = sum(adv_ctc) 

* make HH-level variable for estimated total CTC amount
bys year hrhhid hrhhid2: egen h_ctc = sum(actc_crd)

* refund amount variable
g h_ctcref = h_ctc - h_adv 

* Child and dependent care credit
bys year hrhhid hrhhid2: egen h_cdc = sum(cdc_crd)
...
gen ctc_elig = (h_ctc>0)
gen cdc_elig = (h_cdc>0)
gen child_refund = (h_refund>0)
bys year hrhhid hrhhid2: egen ctc_hh = max(ctc_elig)
bys year hrhhid hrhhid2: egen cdc_hh = max(cdc_elig)
bys year hrhhid hrhhid2: egen ref_hh = max(child_refund)
...
bys year hrhhid hrhhid2: egen h_tot_income = mean(h_tot_income_calc)
recode h_tot_income (0/24999 = 1) (25000/49999 = 2) (50000/99999 = 3) (100000/149999 = 4) (150000/. = 5), generate(inc_cat)
bys year hrhhid hrhhid2: egen income_cat = max(inc_cat)
...
table (income_cat) (month) if h_numkids>1, stat(mean LF) nformat(%5.3fc)
