* THis file imputes values for variables ftotval, fam_agi, and fearnval, using the Multiple Imputation by Chained Equations approach (MICE).
...
*have to do Monotone bc my pattern of missingess is monotone. In this case, it's equivalent to what chained would do.
mi impute monotone (pmm, knn(3)) fearnval ftotval fam_agi  = i.race i.educ i.sex i.hefaminc c.numadult_bin c.numkid_bin i.marital i.spousemp, add(5) rseed(4409) by(marital) force

save mult_imputed_`x', replace
}


use mult_imputed_2019, clear
append using mult_imputed_2020, force
append using mult_imputed_2021, force


save mice_imputed, replace
